...
import { Warehouse, MapPin, CheckCircle2, AlertCircle, X } from 'lucide-react';
...
const CreateHub = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Check if any required field is empty for the global popup
    const isAnyEmpty = !formData.name.trim() || !formData.location.trim() || !formData.latitude.trim() || !formData.longitude.trim();
    ...
    // Success logic
    console.log('Hub Data Submitted:', formData);
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 animate-fade-in relative">
      ...
      <div className="mb-8">
        ...
        <p className={`text-sm font-medium mt-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Register a new central hub to the system.</p>
      </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <textarea 
                ...
                onChange={handleChange}
                rows="3"
                className={`${inputClasses('location')} pl-10 resize-none`}
                placeholder="Full address of the hub"
              />
            </div>
            {errors.location && <p className="text-xs font-bold text-rose-500 mt-1">{errors.location}</p>}
          </div>
