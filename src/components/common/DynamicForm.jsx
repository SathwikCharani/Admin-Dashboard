  ...
  const handleSubmit = async (e) => {
    ...
    if (status === 'Pending') {
      ...
      if (isHubAdmin && title.includes('Store')) {
        import('@/utils/common/emailMock').then(({ sendStoreRequestToSuperEmail }) => {
      ...
      if (isSuperAdmin) {
        const { sendDirectCreationEmail } = await import('@/utils/common/emailMock');
...
const SingleEntryForm = ({ config }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (status === 'Pending') {
      ...
      // TRIGGER EMAIL FOR SUPER ADMIN DIRECT CREATION
      if (isSuperAdmin) {
        import('@/utils/common/emailMock').then(({ sendDirectCreationEmail }) => {
